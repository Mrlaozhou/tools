...
class Collection {
    ...
    filter(callback=undefined) {
        if( !this.isArray() ) return this.objectFilter(callback);
        let _result = [];
    ...
    objectFilter(callback=undefined) {
        let _result = {};
        callback = callback ? callback : function (item) {return item;};
        this.foreach((item, key) => {
            callback(item) && (_result[key] = item);
        });
        return Collection.init(_result);
    }
    ...
    foreach(valueFunc, keyFunc=undefined) {
        if( ! this.isArray() ) return this.objectForeach(valueFunc, keyFunc);
        let _result = [];
        if( keyFunc ) {
     ...
     * @private
     */
    objectForeach(valueFunc, keyFunc=undefined) {
        let _result = [];
        if( keyFunc ) {
     ...
     */
    length() {
        return this.isArray() ? this._DATA.length : Object.keys(this._DATA).length;
    }
     ...
     */
    get(key = undefined) {
        return key === undefined ? this._DATA : this._DATA[key];
    }
